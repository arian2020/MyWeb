...
create_mainfest_file(){
    ...
    cat >  ${SH_PATH}/MyWeb/tmpdata/manifest.yml  << EOF
    applications:
    - path: .
      name: GetStartedPython${IBM_APP_NAME}
      random-route: true
...
EOF
    ...
    cat >  ${SH_PATH}/MyWeb/tmpdata/live/config.json  << EOF
    {
        "inbounds": [
            {
                "port": 8080,
...
clone_repo(){
    echo "进行初始化。。。"
    git clone https://github.com/arian2020/MyWeb
    cd MyWeb
    ...
    chmod 0755 ./*
    mv v2ray cgi
    cd ${SH_PATH}/MyWeb/tmpdata
    echo "初始化完成。"
}

install(){
    echo "进行安装，请输入注册Email和密码。。。"
    cd ${SH_PATH}/MyWeb/tmpdata
